...
Median appears with one or more latency samples. P95 requires 20 samples and P99 requires 100. Insufficient samples are unavailable, not zero. Live polling and WebSocket updates report bounded aggregate counters and the percentiles available at the current sample thresholds. The terminal snapshot finalizes those values. Completed exports include the exact runtime scenario snapshot, build metadata, collision model, timestamps, seed, generated records, aggregate metrics, and failures.
