...
Median appears with one or more latency samples. P95 requires 20 samples and P99 requires 100. Insufficient samples are unavailable, not zero. Live polling and WebSocket updates report bounded aggregate counters; live percentiles use the most recent 2,048 receiver deliveries. The terminal snapshot recalculates exact percentiles over the full run. Completed exports include the exact runtime scenario snapshot, build metadata, collision model, timestamps, seed, generated records, aggregate metrics, and failures.
