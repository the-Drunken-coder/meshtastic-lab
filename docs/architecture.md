...
Results use a temporary file followed by an atomic rename under `/data/runs`. Traffic requests may schedule at most 10,000 messages. Terminal result cloning, serialization, and file writes run in a worker thread so large exports do not block the event loop. Result and summary documents use `schemaVersion: 1`; readers treat an absent version as the original version 1 format. Results are frozen before the rename, and later firmware packets cannot change them. If persistence fails, the last eight unpersisted terminal results remain available through the API until the backend restarts. The polled live summary contains only bounded counters and aggregate maps; full generated-message records stay in the completed export. No packet or UI queue is unbounded.
