...
The backend event loop owns gateways, medium loops, traffic scheduling, lifecycle state, and WebSocket subscriptions. Blocking official-client configuration calls run in worker threads. Each WebSocket subscriber has a bounded 256-event queue; slow clients receive a dropped-event notice while bounded absolute metric updates remain authoritative. Each event has `schemaVersion: 1` and one broker-assigned sequence. A reconnect supplies its last sequence, subscribes first, then replays retained events after that cursor. If the cursor predates the bounded 5,000-event history, the server sends a history-gap notice and the UI refreshes the authoritative lifecycle, scenario, node, and traffic snapshots.
