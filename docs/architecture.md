...
The lifecycle lock serializes commands. Node identity, roles, RF configuration, and channel configuration change only in `STOPPED`. Reset uses the same lock. It restores the default five-node full mesh and clears packet evidence and archived in-memory daemon logs without deleting completed run files. Clearing evidence rotates the event stream so every connected browser discards its local event cache. Directed links use a copy-on-write map under an async lock and can change in `RUNNING`, including during traffic. Every run keeps its starting scenario plus the event sequence, monotonic time, and directed link for each in-run change. Start is all-or-nothing. Configuration writes use the official settings transaction, intentionally reboot once, reconnect the gateway, read effective values back, and fail if they differ.
...
The backend event loop owns gateways, medium loops, traffic scheduling, lifecycle state, and WebSocket subscriptions. Blocking official-client configuration calls run in worker threads. Each WebSocket subscriber has a bounded 256-event queue; slow clients receive a dropped-event notice while bounded absolute metric updates remain authoritative. Each event has `schemaVersion: 1`, an opaque per-process stream ID, and one broker-assigned sequence. A reconnect supplies the stream ID and its last sequence, subscribes first, then replays retained events after that cursor. A changed stream clears the old client history before replay. If the sequence predates the bounded 5,000-event history, the server sends a history-gap notice. The UI refreshes the authoritative lifecycle, scenario, node, and traffic snapshots after either notice.
...
Results use a temporary file followed by an atomic rename under `/data/runs`. Traffic requests may schedule at most 10,000 messages. One request can contain named flows with independent source sets, rates, source timing, and destination policies. The controller merges those schedules on one monotonic timeline and correlates every message to its flow. Terminal result cloning, serialization, and file writes run in a worker thread so large exports do not block the event loop. Result and summary documents use `schemaVersion: 1`; readers treat an absent version as the original version 1 format. Results are frozen before the rename, and later firmware packets cannot change them. The controller does not publish a terminal state until persistence finishes, so a terminal status and export readiness are one API invariant. If persistence fails, the last eight unpersisted terminal results remain available through the API until the backend restarts. The polled live summary contains only bounded counters and aggregate maps; full generated-message records stay in the completed export. It also reports generation, drain, and finalization phase plus pending admission, unresolved direct-message, and drain-deadline values. No packet or UI queue is unbounded.
